...
import { View, Text, StyleSheet, KeyboardAvoidingView, Platform, ActivityIndicator, ScrollView, Pressable, TextInput } from "react-native";
...
function parseFragmentParams(fragment: string): Record<string, string> {
  ...
  for (const pair of clean.split("&")) { const [key, val] = pair.split("="); if (key && val) params[decodeURIComponent(key)] = decodeURIComponent(val); }
  return params;
}
...
export default function ResetPasswordScreen() {
    ...
    const verifyToken = async () => {
      try {
        ...
        if (initialUrl) {
          ...
          const fragParams = parseFragmentParams((parsedUrl as any).fragment || "");
          if (fragParams.access_token) { accessToken = fragParams.access_token; refreshToken = fragParams.refresh_token || null; }
          if (!accessToken && parsedUrl.queryParams?.access_token) { accessToken = parsedUrl.queryParams.access_token as string; refreshToken = (parsedUrl.queryParams.refresh_token as string) || null; }
        }
        if (!accessToken && params.access_token) { accessToken = params.access_token as string; refreshToken = (params.refresh_token as string) || null; }
        if (!accessToken) { setError("Invalid or missing reset token. Please request a new reset link."); setVerifying(false); return; }
        ...
        if (sessError) { setError("Invalid or expired reset link. Please request a new one."); }
      } catch { setError("Failed to verify reset token. Please request a new reset link."); } finally { setVerifying(false); }
    };
    verifyToken();
  }, [params]);
  ...
  const onSubmit = async () => {
    ...
    } catch (e: any) { setError(e?.message || "Failed to update password"); } finally { setLoading(false); }
  };
    ...
    <View style={styles.root}>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.kb}>
        <ScrollView contentContainerStyle={[styles.scroll, { paddingTop: insets.top + 60 }]} keyboardShouldPersistTaps="handled">
          <View style={styles.heroWrap}><View style={styles.logoRing}><Text style={styles.logoMonogram}>M</Text></View><Text style={styles.brand}>MEZBAAN</Text><Text style={styles.brandSub}>RESTRO</Text></View>
          <View style={styles.card}>
            {verifying ? (<View style={styles.centerContent}><ActivityIndicator size="large" color={COLORS.gold} /><Text style={styles.cardSub}>Verifying reset link...</Text></View>)
            : success ? (<View style={styles.centerContent}><Ionicons name="checkmark-circle" size={48} color={COLORS.gold} /><Text style={styles.cardTitle}>Password Updated</Text><Text style={styles.cardSub}>Your password has been changed. Redirecting to login...</Text></View>)
            : (<>
              <Pressable onPress={() => router.replace("/(auth)/login")} style={styles.backBtn}><Ionicons name="arrow-back" size={20} color={COLORS.gold} /><Text style={styles.backTxt}>Back to Login</Text></Pressable>
              <Text style={styles.cardTitle}>Set New Password</Text>
              <Text style={styles.cardSub}>Enter your new password below</Text>
              <View style={styles.inputRow}><Ionicons name="lock-closed-outline" size={20} color={COLORS.textMuted} style={{ marginHorizontal: SPACING.sm }} /><TextInput placeholder="New password (min 6 characters)" placeholderTextColor={COLORS.textMuted} secureTextEntry autoCapitalize="none" autoCorrect={false} value={password} onChangeText={setPassword} style={styles.input} onSubmitEditing={onSubmit} /></View>
              <View style={styles.inputRow}><Ionicons name="lock-closed-outline" size={20} color={COLORS.textMuted} style={{ marginHorizontal: SPACING.sm }} /><TextInput placeholder="Confirm new password" placeholderTextColor={COLORS.textMuted} secureTextEntry autoCapitalize="none" autoCorrect={false} value={confirmPassword} onChangeText={setConfirmPassword} style={styles.input} onSubmitEditing={onSubmit} /></View>
              ...
              <Pressable onPress={onSubmit} disabled={loading || !password || !confirmPassword} style={({ pressed }) => [styles.cta, pressed && { transform: [{ scale: 0.98 }] }]}>
                {loading ? <ActivityIndicator color={COLORS.black} /> : (<><Text style={styles.ctaText}>Update Password</Text><Ionicons name="checkmark" size={20} color={COLORS.black} /></>)}
              </Pressable>
            </>)}
          </View>
        </ScrollView>
