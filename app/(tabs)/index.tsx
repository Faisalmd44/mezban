...
type Item = {
  ...
  description: string; is_bestseller?: boolean; rating?: number; prep_time?: number; created_at?: string;
};
...
export default function Home() {
  ...
  const popular = items.filter((i) => i.is_bestseller);
  ...
  const recommended = items.filter((i) => i.is_bestseller).slice(0, 5);
            ...
            {popular.map((it) => (
              <Pressable key={it.id} testID={`popular-${it.id}`} style={styles.popCard} onPress={() => openProduct(it.id)}>
                <View style={styles.popImgWrap}>
                  ...
                  {it.is_bestseller ? <View style={styles.popBadgeWrap}><BestsellerBadge /></View> : null}
                </View>
                <View style={styles.popBody}>
                  <View style={styles.vegRow}><VegBadge veg={it.is_veg} /><Text style={styles.popCat}>{it.category}</Text></View>
