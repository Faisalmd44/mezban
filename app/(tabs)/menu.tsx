...
type Item = {
  id: string; name: string; category: string; price: number; image: string; is_veg: boolean;
  description: string; in_stock: boolean; variants: { name: string; price: number }[];
  is_bestseller?: boolean; rating?: number; prep_time?: number;
};
...
export default function MenuScreen() {
  ...
  const filtered = useMemo(() => {
    ...
    if (filter === "veg") list = list.filter((i) => i.is_veg);
    else if (filter === "nonveg") list = list.filter((i) => !i.is_veg);
    else if (filter === "popular") list = list.filter((i) => i.is_bestseller);
    ...
    else sorted.sort((a, b) => Number(b.is_bestseller || false) - Number(a.is_bestseller || false));
    return sorted;
  ...
  return (
    <Pressable testID={`menu-item-${item.id}`} onPress={onPress} style={styles.row}>
      <View style={{ flex: 1, paddingRight: SPACING.md }}>
        <View style={styles.badgeRow}><VegBadge veg={item.is_veg} />{item.is_bestseller ? <BestsellerBadge /> : null}</View>
